...
@section('title', 'Buat Wisata')

@section('content')
<div class="max-w-4xl mx-auto">
    <!-- Header -->
    <div class="mb-6">
        <h3 class="text-3xl font-bold bg-gradient-to-r from-sky-600 to-cyan-600 bg-clip-text text-transparent mb-2">Buat Wisata Baru</h3>
        <p class="text-gray-600">Isi form di bawah untuk menambahkan destinasi wisata baru</p>
    </div>
    ...
    <form action="{{ route('admin.wisatas.store') }}" method="POST" enctype="multipart/form-data" class="bg-white/90 backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-sky-100">
        @csrf

        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                <label class="block text-sm font-bold text-gray-700 mb-2">Judul Wisata *</label>
                <input type="text" name="title" id="title" value="{{ old('title') }}" placeholder="Contoh: Curug Cipendok" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-sky-500 focus:ring-4 focus:ring-sky-100 transition-all outline-none" required>
            ...
            <div>
                ...
                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" placeholder="curug-cipendok" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 bg-gray-50 focus:border-sky-500 focus:ring-4 focus:ring-sky-100 transition-all outline-none" required readonly>
        ...
        <div class="mb-6">
            ...
            <textarea name="description" rows="8" placeholder="Ceritakan tentang destinasi wisata ini..." class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-sky-500 focus:ring-4 focus:ring-sky-100 transition-all outline-none resize-none">{{ old('description') }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="text" name="location" value="{{ old('location') }}" placeholder="Desa Karangtengah, Cilongok, Banyumas" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-sky-500 focus:ring-4 focus:ring-sky-100 transition-all outline-none">
            ...
            <div>
                ...
                <input type="number" name="price" value="{{ old('price', 0) }}" placeholder="0" class="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-sky-500 focus:ring-4 focus:ring-sky-100 transition-all outline-none" min="0" step="1">
        ...
        <div class="flex items-center justify-between pt-6 border-t-2 border-gray-100">
            <label class="inline-flex items-center cursor-pointer">
                <input type="checkbox" name="published" value="1" checked class="w-5 h-5 text-sky-600 border-2 border-gray-300 rounded focus:ring-4 focus:ring-sky-100">
                <span class="ml-3 text-sm font-bold text-gray-700">📢 Publikasikan sekarang</span>
            </label>
